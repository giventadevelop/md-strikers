...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const apiUrl = `${API_BASE_URL}/api/event-sponsors-join${queryString ? `?${queryString}` : ''}`;

    console.log('[EventSponsorsJoinProxy] Forwarding to backend URL:', apiUrl);
    ...
    if (method === 'GET') {
      ...
      try {
        ...
        console.log('[EventSponsorsJoinProxy] Parsed request data:', JSON.stringify(requestData, null, 2));
      } catch (parseError) {
        console.error('[EventSponsorsJoinProxy] Failed to parse request body:', parseError);
      ...
      console.log('[EventSponsorsJoinProxy] Final payload with tenantId:', JSON.stringify(payload, null, 2));
    ...
    console.error('EventSponsorsJoinProxy index error:', err);
